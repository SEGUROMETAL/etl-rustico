...
ORDER BY
    (Op, CodRama, Poliza, Supl, Comp, CodCoberturaAut) SETTINGS index_granularity = 8192;


--
CREATE TABLE IF NOT Exists primas_automotores_agg (
    -- Dimensiones (columnas de filtro)
    FEmisionMes Date,
    `CodOrganizador` UInt32,
    `CodProductor` UInt32,
    `CodRama` UInt32,
...
-- Crear la vista materializada que actualiza automáticamente
CREATE MATERIALIZED VIEW IF NOT Exists primas_automotores_agg_mv TO primas_automotores_agg AS
SELECT
    toStartOfMonth(`FEmision`) AS FEmisionMes,
    `CodOrganizador`,
    `CodProductor`,
